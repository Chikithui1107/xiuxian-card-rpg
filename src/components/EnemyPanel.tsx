...
export function EnemyPanel({
  ...
  return (
    <div
      className={`glass-panel-danger relative overflow-hidden p-3 ${
    ...
    >
      ...
      <div
        className={`enemy-sprite-stage relative mx-auto mb-3 flex h-36 w-full max-w-[14rem] items-end justify-center ${
      ...
      >
        {monster ? (
          <img
            src={monster.image}
            alt={monster.name}
            className={`enemy-sprite max-h-full w-auto object-contain drop-shadow-[0_8px_24px_rgba(122,171,154,0.25)] transition-all duration-500 ${
              isDefeated ? "scale-90 opacity-40 grayscale" : ""
            }`}
            draggable={false}
          />
        ) : (
          <div className="flex h-24 w-24 items-center justify-center rounded-full border-2 border-[#8b3a3a]/40 bg-gradient-to-br from-stone-900/80 to-black/70">
